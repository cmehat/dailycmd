...
Names are anonymized (`my-cluster`, `my-app`, `my-bucket`); the same shape works whether `my-app` is a config-file-on-PVC tool, a database, a queue worker — anything with a PVC you want backed up.
...
If this returns `NOT_FOUND` later, the install fails silently — Velero impersonates a GSA that doesn't exist, the metadata server replies `"not defined"`, and every Backup goes `FailedValidation`. This is the most common cause of the silent-failure state.
...
The usual checks — are the pods running? are the CRDs there? are the schedules created? — **can all pass while every Backup silently fails**. The check that actually validates the auth chain is the **BackupStorageLocation phase**. Run:
...
## Test 2 — Destructive in-place test
...
Steps 2-5 are mechanical. Step 1 is the hard part.

### Why step 1 is the hard part
...
- The Helm chart's default `serviceAccount.server.name` is `velero-server`. If you change it, your Workload Identity binding (Step 4) needs to reference the new name — a mismatch produces the same silent `FailedValidation` state.
